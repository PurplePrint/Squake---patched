...
import static gr8pefish.openglider.api.helper.GliderHelper.getIsGliderDeployed;
import static squeek.quakemovement.ToggleKeyHandler.SNEAKING;
...
public class QuakeClientPlayer
{
	...
	public static boolean moveEntityWithHeading(EntityPlayer player, float sidemove, float upmove, float forwardmove)
	{
		...
		/**
		 * PurplePrint's fair play patch
		 * fix the high speed bug (cheat)
		 * https://github.com/squeek502/Squake/issues/34
		 */

		if (ModQuakeMovement.openGliderIntegration) {
			ArrayList gliderItems = new ArrayList<>();
			gliderItems.add(ForgeRegistries.ITEMS.getValue(new ResourceLocation("openglider:hang_glider_basic")));
			gliderItems.add(ForgeRegistries.ITEMS.getValue(new ResourceLocation("openglider:hang_glider_advanced")));
			if (gliderItems.contains(player.inventory.getStackInSlot(player.inventory.currentItem).getItem())) {
				if (getIsGliderDeployed(player)) { return false; }
			}
		}

		if (SNEAKING) {
			if (ModQuakeMovement.techgunsIntegration) {
				ItemStack stackInBackSlot = TGExtendedPlayer.get(player).tg_inventory.func_70301_a(1); // SLOT_BACK = 1
				if (stackInBackSlot != ItemStack.EMPTY) {
					if ((stackInBackSlot.getItem() instanceof ItemJetpack) || (stackInBackSlot.getItem() instanceof ItemGlider)) {
						return false;
					}
				}
			}
		...
		// Too fast swimming patch
//		if (player.isInWater() && !player.isOverWater() && player.isSprinting()) {
		if (player.isInWater()) {
			return false;
		}
